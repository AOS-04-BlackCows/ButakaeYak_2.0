...
class FirestoreManager {
    ...
    fun trySignUp(userData: UserData, resultListener: ResultListener<Boolean>) {
        ...
        db.collection("users")  // 자동 아이디 넣기
            .whereEqualTo("phoneNumber", userData.phoneNumber)
            .get()
            .addOnSuccessListener { documents ->
                if (documents != null && !documents.isEmpty) {
                    // 이미 같은 전화번호로 가입된 사용자가 있음
                    Log.d(TAG, "이미 가입된 번호입니다: ${userData.phoneNumber}")
                    resultListener.onFailure(Exception("이미 가입된 번호입니다."))
                } else {
                    // 중복된 번호가 없을 경우 회원가입 진행
                    db.collection("users")
                        .add(userData.toMap())
